...
{
    public class AuthService(IOptions<InternalApiSettings> settings) : IAuthService
    {
        ...
        private readonly string _url = settings.Value.BaseUrl;
        ...
        public async Task<string> GetAccessTokenAsync()
        {
            ...
            if (Debugger.IsAttached)
            {
                ...
                token = await GetAccessTokenWithManagedIdentityAsync(_url);
        ...
        private static async Task<string> GetTokenAsync(TokenCredential credential, string resourceUrl)
        {
            return (await credential.GetTokenAsync(
                new TokenRequestContext(scopes: [resourceUrl + "/.default"]) { }, new CancellationToken())).Token;
        }
    }
}